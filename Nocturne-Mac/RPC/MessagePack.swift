...
/// Minimal MessagePack codec — enough for the Pi connector's RPC payloads
/// (nil, bool, int, double, string, bin, array, map). The Car Thing's
/// `nocturned` daemon uses the standard msgpack v5 spec with no ext or
/// timestamp types, so we don't need them either.
///
/// Mirrors what `src/server/rpc/msgpack-codec.ts` does via the
...
// MARK: - JSON-like helpers for ergonomic construction
...
extension MessagePackValue {
    /// Bridge a Swift value into a MessagePackValue. Supports nil / Bool /
    /// integer types / Double / Float / String / Data / Array / Dictionary
    /// (with String keys) and nested combinations thereof.
    static func wrap(_ value: Any?) -> MessagePackValue {
        ...
        case let n as NSNumber:
            // Be careful: NSNumber bridges Bool as well; check that first.
