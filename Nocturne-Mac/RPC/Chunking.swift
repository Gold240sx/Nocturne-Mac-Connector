...
enum Chunking {
    ...
    /// Splits `data` into one or more chunks tagged with `messageId`.
    ...
    /// Tries to parse one chunk from the front of `buffer`. Returns `.needMoreData`
    /// if the buffer is incomplete, `.invalid` for unrecoverable framing errors
    /// (caller should drop `dropBytes` from the front and retry), or `.success`
...
/// Reassembles multi-chunk messages by `messageId`.
