...
/// Port of `src/server/rpc/rpc-client.ts` (base64-newline wire format only —
/// that's what the Pi connector uses for the outbound RFCOMM channel and what
/// the Car Thing's `nocturned` daemon speaks).
///
/// Each outbound message is encoded with msgpack, chunked, base64'd, and
/// terminated with a newline. Incoming bytes are split on newlines and each
/// line decoded either as raw msgpack or as a chunk header + payload.
@MainActor
final class RPCClient {
    ...
    /// Feed inbound bytes from the RFCOMM channel.
    ...
    private func processBuffer() async {
        while true {
            // Find a newline (0x0A) — frames are line-delimited.
            ...
            // Try raw msgpack first (small messages); fall back to chunk framing.
    ...
    /// Fire-and-await an outbound call. Resolves with the remote's result or
    /// throws on remote error / timeout.
    @discardableResult
    ...
    /// Send an outbound event (no response expected).
