...
/// One-shot HTTP listener bound to `127.0.0.1` to catch a Spotify OAuth
/// redirect like `http://127.0.0.1:8888/callback?code=...&state=...`.
///
/// Started right before opening the browser; the first GET that lands gets
/// a small "Authorized!" HTML response, then the listener stops itself and
/// the parsed query params are delivered via the awaiting continuation.
@MainActor
final class LocalhostCallbackServer {
    ...
    /// Holds open connections so the system doesn't tear them down mid-reply.
    private var connections: [NWConnection] = []

    init(port: UInt16) { self.port = port }

    /// Bind and start accepting. Throws if the port is already in use.
    func start() throws {
    ...
    /// Await the first request's parsed query parameters. The continuation
    /// is fulfilled exactly once — subsequent requests are still served the
    /// success HTML but their params are discarded.
    func waitForCallback(timeout: TimeInterval = 300) async throws -> [String: String] {
        return try await withCheckedThrowingContinuation { cont in
            self.continuation = cont
            // Safety timeout — if the user closes the browser without finishing,
            // we don't leak an infinite-await Task.
            Task { @MainActor [weak self] in
    ...
    private func respondSuccess(on conn: NWConnection) {
        ...
        <p>You can close this tab and return to Nocturne-Mac.</p></div></body></html>
        """
